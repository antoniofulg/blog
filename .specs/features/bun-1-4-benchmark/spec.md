...
| Machine-contention control | The harness records load average, free memory, host, CPU model and battery/AC state into every result file, and refuses to run unless `--allow-noisy` is passed when 1-minute load average exceeds 2.0. | Cheap guard that keeps an accidental noisy run from being mistaken for a publishable one; the escape hatch keeps it non-blocking. | n |
...
5. IF the 1-minute load average exceeds 2.0 and `--allow-noisy` was not passed THEN the system SHALL abort before running any workload with a message naming the observed load average.
