...
- **`schema.parse(...)`** é a coleira. Se o modelo inventar um campo, esquecer outro ou mandar um tipo errado, o Zod estoura ali — você descobre na hora, não três telas depois com um `undefined` misterioso.
...
export function createMessageGeneratorNode(llm: OpenRouterService) {
  return async (state: GraphState): Promise<Partial<GraphState>> => {
    ...
    const scenario = `${state.intent ?? "unknown"}_${hasSucceeded}` // ex.: "schedule_success"
