...
> ⚠️ `POST` não é totalmente incacheável: a [RFC 9110 §9.3.3](https://www.rfc-editor.org/rfc/rfc9110#section-9.3.3) permite cachear a resposta de um `POST` quando ela traz informação explícita de frescor (*freshness*) e um `Content-Location` igual à URI da requisição — mas a cópia em cache só pode ser reusada por um `GET`/`HEAD` posterior, e quase nenhum cache faz isso na prática. O `QUERY` é cacheável como ele mesmo, indexado pelo corpo da requisição.
