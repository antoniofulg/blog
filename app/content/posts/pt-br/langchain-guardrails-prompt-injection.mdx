...
const agent = createAgent({
  ...
  middleware: [
    ...
    // 2. não vaza telefone/e-mail de cliente na resposta
...
Três defesas independentes, executadas na ordem da lista:

- **`injectionGuard`** barra o recado malicioso na entrada (o nosso porteiro).
- **`piiRedactionMiddleware`** raspa dado pessoal _da saída_ — mesmo que algo escape, telefone de cliente não sai escrito na resposta.
- **`humanInTheLoopMiddleware`** põe um humano no meio antes de uma ação irreversível. "Cancela todos os horários de amanhã" não executa sozinho — para e pede um `approve`. É o equivalente a exigir duas chaves pra abrir o cofre.
