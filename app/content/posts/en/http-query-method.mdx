...
> ⚠️ `POST` isn't flatly uncacheable: [RFC 9110 §9.3.3](https://www.rfc-editor.org/rfc/rfc9110#section-9.3.3) lets a `POST` response be cached when it carries explicit freshness information and a `Content-Location` equal to the request URI — but the cached copy is only reusable for a later `GET`/`HEAD`, and virtually no cache implements it. `QUERY` is cacheable as itself, keyed on the request body.
